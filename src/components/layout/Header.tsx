...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          <details className="group relative">
            <summary className="flex cursor-pointer list-none items-center gap-1 rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 [&::-webkit-details-marker]:hidden">
              Tools
              <ChevronDownIcon className="h-4 w-4 transition-transform group-open:rotate-180" />
            </summary>
            <div className="absolute right-0 z-50 mt-2 grid w-72 grid-cols-1 gap-1 rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
              {tools.map((tool) => (
                <Link
                  key={tool.slug}
      ...
      {mobileOpen && (
        <nav id="mobile-nav" className="border-t border-slate-200 bg-white md:hidden">
