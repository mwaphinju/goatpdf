...
|---|---|---|---|
| `NEXT_PUBLIC_SITE_URL` | **Should be set** | `https://goatpdf.app` (placeholder) | Base URL for canonical links, Open Graph/Twitter tags, `sitemap.xml`, `robots.txt`. If left unset in production, every one of these will silently point at a domain the app doesn't actually own. **`NEXT_PUBLIC_*` variables are inlined at build time, not read at runtime** — for a Docker deployment this must reach the actual `docker build` step (as a `--build-arg`), not just the running container's environment. The Dockerfile declares `ARG NEXT_PUBLIC_SITE_URL` for exactly this reason; a platform that only injects env vars at runtime (not as build args) would silently ship the placeholder regardless of what's configured. Confirmed working end-to-end: built locally with `--build-arg NEXT_PUBLIC_SITE_URL=<test-url>` and verified the resulting page's `<link rel="canonical">`/`og:url` actually reflected it. |
