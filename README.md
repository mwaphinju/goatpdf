...
Every tool page now follows the same shape (title, description, upload, options, processing/success/error states, download, start over, related tools), with shared components for the repeated pieces and a pass for keyboard focus states, touch targets, and reduced-motion support. What's left before launch is infrastructure and content, not tool functionality: SEO metadata/sitemap, a Lighthouse/performance pass, and security hardening + a Dockerfile (with LibreOffice installed) for real deployment. See the "Development phases" section of [CLAUDE.md](./CLAUDE.md) for the current plan and status.
