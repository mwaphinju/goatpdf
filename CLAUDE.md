...
- `soffice` is resolved via `SOFFICE_PATH` (env override) → common Windows install paths → bare `soffice` on `PATH` (the expected case on Linux/Docker). The [Dockerfile](Dockerfile) installs LibreOffice (`apt-get install libreoffice`) in its runtime stage so the bare-`soffice`-on-`PATH` fallback resolves correctly — written but not yet build-tested (no Docker available in the dev environment it was authored in; see [DEPLOYMENT.md](DEPLOYMENT.md)).
...
- **Phase 9.5 — MVP audit, critical-fix pass, and deployment prep** ✅ *done*:
  ...
  - **The [Dockerfile](Dockerfile) and [.dockerignore](.dockerignore) now exist** — multi-stage, `node:20-bookworm-slim` (Debian, not Alpine — LibreOffice + sharp/@napi-rs/canvas are far more reliable there), LibreOffice + `fonts-liberation` installed in the runtime stage, deliberately *not* using `output: "standalone"` (sidesteps the pdfjs-dist tracing gap above entirely rather than working around it), non-root user, `HEALTHCHECK`. **Not yet build-tested** — no Docker available in the environment this was authored in; the first real `docker build` is still an open item.
