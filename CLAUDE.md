...
- `soffice` is resolved via `SOFFICE_PATH` (env override) → common Windows install paths → bare `soffice` on `PATH` (the expected case on Linux/Docker). The [Dockerfile](Dockerfile) installs LibreOffice (`apt-get install libreoffice`) in its runtime stage so the bare-`soffice`-on-`PATH` fallback resolves correctly — **confirmed working**: a real `docker build` + `docker run` successfully converted a real PDF to a genuine `.docx` through the containerized LibreOffice install (see [DEPLOYMENT.md](DEPLOYMENT.md)).
...
- **Phase 9.5 — MVP audit, critical-fix pass, and deployment prep** ✅ *done*:
  - **MVP audit** ([MVP_AUDIT.md](MVP_AUDIT.md)): full automated suite plus live adversarial testing (path traversal, a download race condition, oversized/combined uploads, concurrent + repeated processing, slow-network simulation, a full internal-link/JSON-LD crawl) against a real production build. 0 FAIL, 8 WARNING (all Medium or Low — no High/Critical), 0 blocking defects.
  - **Fix pass**: the two highest-severity findings fixed and re-verified live — SEC-1 (combined-size check now runs before any workspace/disk I/O, not after) and A11Y-1 (`text-slate-400` contrast failures fixed; footer needed `text-slate-600`, not `text-slate-500`, once its real `slate-50` background was accounted for). A live Playwright test now resolves real rendered color via a canvas pixel read and guards the fix. The remaining 6 Low-severity findings were left open.
  - **Deployment readiness review** ([DEPLOYMENT.md](DEPLOYMENT.md)): environment variables, build configuration, production scripts, security headers, file processing requirements, temp storage, cleanup mechanism, and domain configuration reviewed against the actual code. Surfaced two concrete, non-obvious risks: (1) `pdfRenderer.ts` resolves pdfjs-dist's font/cmap assets via a runtime-built path, not an import — Next's `output: "standalone"` file tracer can't see it, so a naive standalone Docker build would silently mis-render PDFs needing standard-font substitution; (2) job/rate-limit state is per-process in-memory by design, so the app must run as a single instance (or with session affinity) — it is not safe to horizontally scale as-is.
  - **The [Dockerfile](Dockerfile) and [.dockerignore](.dockerignore) now exist and build successfully** — multi-stage, `node:22-bookworm-slim` (Debian, not Alpine — LibreOffice + sharp/@napi-rs/canvas are far more reliable there; Node 22 specifically because `pdfjs-dist@6.3.289` requires it, caught by the real build), LibreOffice + `fonts-liberation` installed in the runtime stage (with `apt-get`'s `Acquire::Retries=5`, added after a real transient DNS failure during a ~150-package install), deliberately *not* using `output: "standalone"` (sidesteps the pdfjs-dist tracing gap above entirely rather than working around it), non-root user, `HEALTHCHECK`.
  - **Build-tested for real, not just written**: `docker build` succeeds, and the resulting image was actually run and smoke-tested — homepage 200 with all security headers present; PDF to Word produced a genuine `.docx` through the container's real LibreOffice install; PDF to JPG produced a valid 4-page ZIP of real JPEGs (the concrete proof the `standard_fonts`/`cmaps` asset directories — 16 and 169 files respectively — are actually present and working in the image); Merge PDF succeeded; temp-workspace cleanup-on-download was confirmed live inside the running container with correct non-root/`0700` ownership throughout; `HEALTHCHECK` reported `healthy`.
  - **Vercel evaluated and rejected** for this app's current architecture (documented in DEPLOYMENT.md): no way to install LibreOffice in Vercel's serverless sandbox, and the in-memory job/rate-limit state doesn't survive Vercel's per-invocation instance model. Docker-on-a-PaaS (Render/Railway/Fly.io-style, per the original architecture decision) remains the plan.
- **Phase 8 — Pre-launch** (remaining): build-test the Dockerfile for real, a cleanup-mechanism stress test under sustained load, a full E2E pass against the built container (not just `next start` on bare metal), then deploy to the chosen Docker PaaS and do a production smoke test. (`npm audit` — 0 vulnerabilities — has already been run repeatedly as part of Phase 9.5 and the MVP audit; not a separate remaining step.)
